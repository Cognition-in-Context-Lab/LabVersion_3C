...
#convert factors classes into numeric classes

### should this be calcualted using the omit_df and not df? We should move this to the other class transformations regardless. 
...
### should we calculate this after we create the omit_df dataframe? 
...
#view(unique(omit_df$Age))
#as.numeric(levels(omit_df$Age))[as.integer(omit_df$Age)]
mean(as.numeric(levels(omit_df$Age))[as.integer(omit_df$Age)], na.rm = T)
sd(as.numeric(levels(omit_df$Age))[as.integer(omit_df$Age)], na.rm = T)
table(omit_df$Gender)
table(factor(omit_df$Gender,labels=c("male","female","nonbinary","prefer not to answer","trans man","genderqueer"))) #these are the ones that are not 0
table(df$Race)
table(df$Education)
```
...
describe(omit_df$MFQ_part1_sum)
describe(omit_df$MFQ_part2_sum)
...
describe(omit_df$Percevied_Risk_Sum) #confused about the sum?? probably just need to restart from beginning so that excel is normal (we dont want omit_df to have Sum column!)
```
