...
df[which(df$'Duration..in.seconds.' >= 45*60), ]  # 4 entries

# return entries where duration was shorter than 6 mins.
df$'Duration..in.seconds.' = as.numeric(as.character(df$'Duration..in.seconds.'))
df[which(df$'Duration..in.seconds.' <= 360), ]   # 29 entries
...
df <- df[-c(1:7,8:12,22,205,222,255,324,415:444), ]  #removed 47 entries
...
#can subset(omit_df, PandemicLengthExpect.1 >= 3486.55 && COVID_DoG_index  >= 1433.28)

omit_df <- omit_df %>% slice(-c(23, 118, 176, 191, 243, 252, 270, 323, 331, 350, 364, 392))

#380 participants are now left out of the original 397, but it seems like it is not removing the data we actually want to?
...
omit_df <- omit_df %>% mutate(MCQ_Sum = rowSums(mini_df), 
                              .before = COVID_DoG_Questions.1_1)
omit_df <- omit_df %>% mutate(MCQ_index = rowMeans(mini_df), 
                              .before = COVID_DoG_Questions.1_1)
```
...
omit_df <- omit_df %>% mutate(Norms_Sum = rowSums(mini_df), 
...
omit_df <- omit_df %>% mutate(IUS_Sum = rowSums(mini_df), 
                              .before = MFQpart1_1)
omit_df <- omit_df %>% mutate(IUS_index = rowMeans(mini_df),
                              .before = MFQpart1_1)
```
...
omit_df <- omit_df %>% mutate(Measure2_PO_Sum = rowSums(mini_df), 
                              .before = Measure5_1)
omit_df <- omit_df %>% mutate(Measure2_PO_index = rowMeans(mini_df),
                              .before = Measure5_1)
# measure 5
mini_df <- omit_df %>% select(Measure5_1:Measure5_14)
omit_df <- omit_df %>% mutate(Measure5_PO_Sum = rowSums(mini_df))
omit_df <- omit_df %>% mutate(Measure5_PO_index = rowMeans(mini_df))
#combining both measures
mini_df <- omit_df %>% select(contains("PO_Sum"))
#mini_df
omit_df <- omit_df %>% mutate(PO_Total_Sum = rowSums(mini_df))
mini_df <- omit_df %>% select(contains("PO_index"))
omit_df <- omit_df %>% mutate(PO_total_index = rowMeans(mini_df))
```
...
omit_df <- omit_df %>% mutate(MFQ_sum_part1 = rowSums(MFQ_part1), 
                              .before = MFQ_part2_1)
omit_df <- omit_df %>% mutate(MFQ_sum_part2 = rowSums(MFQ_part2), 
                              .before = Disgust_1)
omit_df <- omit_df %>% mutate(MFQpart1_index = rowMeans(MFQ_part1), 
                              .before = MFQ_part2_1)
omit_df <- omit_df %>% mutate(MFQpart2_index = rowMeans(MFQ_part2), 
                              .before = Disgust_1)
#view(omit_df)
MFQ_total <- omit_df %>% select(contains("MFQ_sum"))
#MFQ_total
omit_df <- omit_df %>% mutate(MFQ_total_sum = rowSums(MFQ_total), 
                              .before = Disgust_1)
omit_df <- omit_df %>% mutate(MFQtotal_index = rowMeans(MFQ_total),
                              .before = Disgust_1)
```
...
#mini_df
omit_df <- omit_df %>% mutate(Stress_Sum = rowSums(mini_df), 
...
#Experiences
omit_df <- omit_df %>% mutate(Impacts_sum = rowSums(Impacts), 
                              .before = PersonalDiagnoses_1)
omit_df <- omit_df %>% mutate(Experiences_sum = rowSums(Experiences),
                              .before = Age)
...
omit_df <- omit_df %>% mutate(Disgust_Sum = rowSums(mini_df), 
                              .before = SDRS.5_Q1)
omit_df <- omit_df %>% mutate(Disgust_index = rowMeans(mini_df), 
                              .before = SDRS.5_Q1)
...
omit_df <- omit_df %>% mutate(SDRS_Sum = rowSums(mini_df), 
                              .before = Worries_1)
omit_df <- omit_df %>% mutate(SDRS_index = rowMeans(mini_df), 
                              .before = Worries_1)
#view(omit_df)
```
...
#original df dataset had 444 rows, removed 47 rows in Load and Cleanup Data section (unneeded rows, incomplete trials, pilot data, and too short or too long entries) = 397 rows 
#omit_df dataset had 397 rows, removed 6 entries with missing data 
#another 12 were removed for responses that were 3SD above folks average COVID DoG or COVID expexted pandemic ength 
nrow(omit_df) # 380 rows
# thus, we excluded 20 participants (from 400 collected)
