...
hist(omit_df$Norms_index, breaks = 50) 
...
hist(omit_df$GCS_index, breaks = 50) 

```

# Intolerance of Uncertainty (sum score, with one factor, or two factors (1-7 for prospective anexity + 8-12 for inhibitory anexity))
```{r}
mini_df <- omit_df %>% select(IUS_1:IUS_12)
omit_df <- omit_df %>% mutate(IUS_Sum = rowSums(mini_df), 
                              .before = MFQpart1_1)

hist(omit_df$IUS_index, breaks = 50) 
...
# COVID Stress (Mean across all three portions and three seperate)
```{r}
...
hist(omit_df$Stress_index, breaks = 50) 

#whats going on here?  Why is it graphing like this?
