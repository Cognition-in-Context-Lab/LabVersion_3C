...
```{r}
df <- read.csv("df.csv", header=T, na.strings="")
...
#WZ: another way to get descriptive statistics
describe(omit_df)
```

```{r}
#Age
hist(omit_df$Age) # seems like there's an age outlier; max age is 1971. Someone might have improperly reported age for year?
describe(omit_df$Age)
...
#find how many people put 4 or 5 for each 
#WZ: what does 4 or 5 mean? a variable key would be useful!

#orientation (liberal conservative, libertarian)
length(which(omit_df$Measure2_2 == 4)) + length(which(omit_df$Measure2_2 == 5)) #liberal

length(which(omit_df$Measure2_3 == 4)) + length(which(omit_df$Measure2_3 == 5)) #conservative

length(which(omit_df$Measure2_4 == 4)) + length(which(omit_df$Measure2_4 == 5)) #libertarian

#however, not everyone put a 4 or 5 in their answers. so...

#check which orientation if greatest for each person
omit_df$which_Measure2 = ifelse((omit_df$Measure2_2 > omit_df$Measure2_3) & (omit_df$Measure2_2 > omit_df$Measure2_4), 'liberal',
                      ifelse((omit_df$Measure2_3 > omit_df$Measure2_2) & (omit_df$Measure2_3 > omit_df$Measure2_4), 'conservative', 
                      ifelse((omit_df$Measure2_4 > omit_df$Measure2_2) & (omit_df$Measure2_4 > omit_df$Measure2_2),'libertarian', 'none')))
...
mean(omit_df$Measure2_2)
mean(omit_df$Measure2_3)
mean(omit_df$Measure2_4)

#WZ: might be a good idea to make a table here
omit_df %>% 
  select(X,Measure2_2,Measure2_3,Measure2_4) %>% 
...
summary(GCSModel)
```
WZ: Summarizing results: CS_index does not significantly predict COVID_DoG_index, F(1,376) = 1.726, p=.19. Unsurprising as visually the graph doesn't show a clear relationship. Also, notable that the COVID_DoG_index variable is heavily negatively skewed (i.e. most ppl at higher range of scale), limiting its variance.
...
# WZ: are all these outliers? I only located 2 from my results..
...
WZ: Does this correspond to the hypothesis, "People who report more discounting on the general DoG measure will delay less on the adopted COVID 19 specific DoG Task"?

WZ: Discuss whether to use the log or the unlogged versions of these indices? 

### SmlK
```{r}
#WZ: I always like to make a graph to visualize the data whenever possible.
ggplot(omit_df_final,aes(x=SmlK,y=logCOVID_DoG_index))+
  geom_point()
...
summary(MCQModel)
```
WZ: SmlK does not appear to be a continuous variable. Is it ordinal? If so, might want to code it as a factor instead.

Results suggest that SmlK significantly negatively predicts logCOVID_DoG_Index, F(1,350)=6.284, p=.012. Every one point increase in SmlK predicts a -3.195 decrease in logCOVID_DoG_Index.
...
### MedK
```{r}
#WZ: make a plot first to visualize data
ggplot(omit_df_final,aes(x=MedK,y=logCOVID_DoG_index))+
  geom_point()
...
summary(MCQModel2)
```
WZ: MedK marginally predicts less logCOVID_DoG_index, F(1,350)=2.902, p=.09. Every 1 unit increase in MedK predicts -2.64 change in logCOVID_DoG_index.
...
summary(MCQModel3)
```
WZ: LrgK does not predict logCOVID_DoG_index. 
...
## MCQ and Uncertainty 
WZ: Is this the question being tested here? "People who report less confidence about the future should report more temporal discounting."
...
# WZ: wondering if you mean logMeanK instead of logLrgK?

summary(GCSModel3)
```
WZ: GCS_index did not significantly predict logLrgK, p=.201.
...
summary(GCSModel4)
```
WZ: GCS_index did not predict logMedK, p=.324. 
...
```
#Moderation Models (Political Orientation and DoG, moderated by Impact and Experiences)
